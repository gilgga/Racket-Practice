...
(define (element?-tco s e)
  (if (null? s) #f (if (equal? (car s) e) #t (element?-tco (cdr s) e))))
...
(define (union-tco s t)
  (if (null? s) t
  (if (null? t) s
      (if (element?-tco s (car t)) (union-tco s (cdr t))
...
(define (intersection-tco-help s t rv)
  (if (or (null? s) (null? t)) rv
      (if (element?-tco s (car t)) (intersection-tco-help s (cdr t) (cons (car t) rv))
...
(define (set-difference-tco-help s t rv)
  (if (null? s) rv
      (if (not (element?-tco t (car s))) (set-difference-tco-help (cdr s) t (cons (car s) rv))
