...
(define (map f lst)
  (if (equal? lst '()) '() (cons (f (car lst)) (map f (cdr lst)))))
...
(define (filter pred lst)
  (if (equal? lst '()) '() 
   (if(equal? (pred (car lst)) #t) (cons (car lst) (filter pred (cdr lst))) (filter pred (cdr lst)))))
...
(define (sum lst)
  (if (equal? lst '()) 0 (+ (car lst) (sum (cdr lst)))))
...
(define (product lst)
  (if (equal? lst '()) 1 (* (car lst) (product (cdr lst)))))
