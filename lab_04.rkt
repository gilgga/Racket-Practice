...
(define (element? item list-of-items)/
  (if (null? list-of-items) #f (if (equal? item (car list-of-items)) #t (element? item (cdr list-of-items) ))))
...
(define (keep-non-set list-of-items)
  (cond
    [(null? list-of-items)
     '()]
    [(element? (car list-of-items) (cdr list-of-items))
     (cons (car list-of-items) (keep-non-set (cdr list-of-items)))]
    [else
     (keep-non-set (cdr list-of-items))]))
...
(define (intersection listA listB)
  (make-set (keep-non-set (append (make-set listA) (make-set listB)))))
...
;; Define superset?
;; A is a superset of B if every element in B is an element of A.
;; i.e. (A >= B) <==> (B <= A)
...
(define (insert element lst)
  (if (element? element lst) (make-set lst) (make-set (cons element lst))))
...
(define (remove element set)
  (if (not(element? element set)) set (set-difference set (list element))))
