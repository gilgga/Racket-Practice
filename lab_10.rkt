...
;; Gil Gerard G. Austria
;; "I pledge my honor that I have abided by the Stevens Honor System." -gaustria
...
(define (M tuplelist)
  (if (null? tuplelist) 1
      (* (car (reverse (car tuplelist))) (M (cdr tuplelist)))))
...
;; To check this check every combination of 2 b's
...
(define (reverse-car tuplelist)
  (car (reverse tuplelist)))

(define (getb tuplelist)
  (if (null? tuplelist) '()
      (cons (reverse-car (car tuplelist)) (getb (cdr tuplelist)) )))

(define (lstrecur num lst)
  (if (null? lst) #t
      (if (not (equal? (EuclidAlgo num (car lst)) 1)) #f
               (lstrecur num (cdr lst)))))

(define (CRT-exists? tuplelist)
  (if (null? tuplelist) #t
      (and (lstrecur (reverse-car (car tuplelist)) (getb (cdr tuplelist)))
...
;; Ai is the first element of each tuple,
;; Mi is m/b where b is the second element of each tuple
;;       m is the product of all b in the list,
;; Mi^-1 is the mul_inv of Mi mod Bi

;; Examples:
;; (CRT-helper '((10 11) (4 12) (12 13)) 1716) -> -17876 = [10*(1716/11)*((1716/11)^-1)] + [4*(1716/12)*((1716/12)^-1)] + [12*(1716/13)*((1716/13)^-1)]
...
(define (CRT-helper tuplelist m)
  (if (null? tuplelist) 0
      (+ (* (* (car (car tuplelist))    (quotient m (reverse-car (car tuplelist))) )  (mul_inv (quotient m (reverse-car (car tuplelist))) (reverse-car (car tuplelist))) )
...
(define (adduntilpos num adder)
  (if (positive? num) num
      (adduntilpos (+ num adder) adder)))

(define (CRT tuplelist)
  (if (not (CRT-exists? tuplelist)) -1
      (adduntilpos (CRT-helper tuplelist (M tuplelist)) (M tuplelist))))
